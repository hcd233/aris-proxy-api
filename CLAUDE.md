...
- **`internal/infrastructure/database/model/`** — GORM 模型。`BaseModel` 包含 ID/CreatedAt/UpdatedAt/DeletedAt。关键模型：User、Message（存储 UnifiedMessage JSON + SHA256 CheckSum）、Session（跟踪 APIKeyName + MessageIDs + ToolIDs 为 JSON 数组，含 Summary/Scores 字段）、Tool（存储 UnifiedTool JSON + CheckSum）、ModelEndpoint（存储上游模型路由配置）、ProxyAPIKey（存储代理 API Key）
...
- **`internal/common/model/`** — 通用数据模型（错误响应、参数结构体）
