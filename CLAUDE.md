...
- **`internal/api/`** — Fiber App 和 Huma API 单例，通过 `init()` 初始化。Fiber 使用 Sonic 作为 JSON 编解码器。Huma 注册 `jwtAuth` 和 `apiKeyAuth` 两种安全方案
- **`internal/router/`** — 按领域分组的路由注册。每个文件绑定中间件和 Handler，使用 `huma.Register()` 注册操作
- **`internal/handler/`** — 每个 Handler 为**接口**，私有结构体实现，通过 `NewXxxHandler()` 创建。Handler 持有 Service 引用，用 `util.WrapHTTPResponse()` 包装响应。流式响应（SSE/LLM）返回 `*huma.StreamResponse`
- **`internal/service/`** — 业务逻辑层。LLM 代理 Service 处理上游请求构建、SSE 流式转发、模型名替换和异步消息存储
- **`internal/infrastructure/database/dao/`** — 泛型 `baseDAO[ModelT]` 提供类型安全的 CRUD、分页和批量操作。具体 DAO 嵌入它。所有 DAO 为单例。软删除使用 `deleted_at`（int64 时间戳，0 = 未删除）
- **`internal/infrastructure/database/model/`** — GORM 模型。`BaseModel` 包含 ID/CreatedAt/UpdatedAt/DeletedAt。关键模型：User、Message（存储 UnifiedMessage JSON + SHA256 CheckSum）、Session（跟踪 APIKeyName + MessageIDs + ToolIDs 为 JSON 数组）、Tool（存储 UnifiedTool JSON + CheckSum）、ModelEndpoint、ProxyAPIKey、Param
- **`internal/dto/`** — 请求/响应 DTO。包含完整的 OpenAI 和 Anthropic API 类型定义，以及 `UnifiedMessage` 和 `UnifiedTool` 跨 Provider 格式及双向转换函数。`MessageContent` 使用自定义 JSON 序列化/反序列化处理联合类型（string | array）
- **`internal/middleware/`** — JWT 解码 Token 并注入 userID/userName/permission 到上下文。APIKey 中间件从代理配置构建反向索引验证 Key 并注入 userName。RateLimiter 使用 Redis + `ulule/limiter`。Lock 中间件使用 Redis SETNX + Lua 脚本原子解锁
- **`internal/infrastructure/pool/`** — `PoolManager` 管理 Pond v2 协程池：`pingPool` 和 `messageStorePool`。消息存储任务通过 SHA256 CheckSum 去重（批量 IN 查询），然后在事务中创建 messages/tools/sessions。使用 `util.CopyContextValues()` 安全传递上下文到异步任务
- **`internal/infrastructure/cache/`** — Redis 缓存单例
- **`internal/infrastructure/storage/`** — 对象存储抽象层（MinIO / COS），通过 `ObjDAO` 接口切换
- **`internal/agent/`** — 可复用的 LLM/Agent 能力（Session 总结器、评分器）
- **`internal/cron/`** — 定时任务调度（会话去重、总结、评分、软删除清理）
- **`internal/config/`** — Viper 环境变量配置加载
- **`internal/common/constant/`** — 项目常量（上下文 Key、HTTP、Agent、时间、速率、字符串）
- **`internal/common/enum/`** — 公共枚举（OAuth2 平台、对象存储、对象类型、权限、排序、SSE 数据/状态等）
- **`internal/common/ierr/`** — 统一错误创建和处理包（哨兵错误 + BizError 模式）
- **`internal/common/model/`** — 通用数据模型（错误响应、参数对象）
- **`internal/enum/`** — 业务枚举（环境、Provider 类型、内容类型、角色、语音、模态等）
- **`internal/jwt/`** — JWT 签发和验证单例
- **`internal/lock/`** — Redis 分布式锁（SETNX + Lua 原子解锁）
- **`internal/logger/`** — Zap 结构化日志（控制台 + 文件多输出 tee，Lumberjack 轮转）
...
Client → /api/openai/v1/chat/completions (model=my-alias)
  → APIKeyMiddleware 从代理配置验证 Key
  → Service 查找 my-alias → 找到 openai endpoint 配置
...
2. **单例模式**：Fiber App、Huma API、DB、Redis、DAO、JWT Signers、PoolManager — 全部初始化一次
3. **泛型 DAO**：`baseDAO[ModelT]` 使用 Go 泛型实现类型安全 CRUD
4. **策略模式**：OAuth2 平台切换（`Platform` 接口）、对象存储平台切换（COS 优先 > MinIO）
5. **统一消息格式**：OpenAI/Anthropic DTO → UnifiedMessage/UnifiedTool 跨 Provider 存储
6. **异步协程池**：LLM 请求后的消息存储通过 Pool，SHA256 CheckSum 去重
7. **上下文感知日志**：`logger.WithCtx(ctx)` 和 `logger.WithFCtx(fctx)` 自动附加 traceID、userID、userName
...
- **Sonic**：高性能 JSON（替代 Fiber 中的 encoding/json）
- **Cobra / Viper**：CLI 和配置
- **Zap** + Lumberjack：结构化日志，多输出 tee（控制台 + info/error/panic 文件轮转）
- **MinIO / Tencent COS**：对象存储（通过 ObjDAO 接口抽象）
- **golang-jwt**：JWT 签发和验证
- **Pond v2**：异步任务协程池
- **robfig/cron/v3**：定时任务调度
- **ulule/limiter**：Redis 限流
- **samber/lo**：Go 函数式工具
- **Eino**（cloudwego/eino）：AI 框架（Agent 编排、LLM 集成）
