...
所有常量必须定义在 `constant/` 或 `enum/` 中，通过 import 引用。
...
**禁止在 `constant/`、`enum/` 中定义 `const X = pkg.Y` 形式的转发封装。**
...
**DTO 中的时间字段使用 `time.Time` 类型，禁止在 Service 层格式化为字符串。**
...
以下路径不参与魔法值扫描：
- `internal/common/constant/` / `internal/common/enum/` / `internal/enum/` / `internal/common/ierr/`
- `internal/infrastructure/` / `internal/config/` / `internal/logger/`
