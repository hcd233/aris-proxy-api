...
type callbackCase struct {
	...
	ExpectIsNew bool   `json:"expected_is_new"`
	ExpectError string `json:"expect_error_kind"`
}
...
func newStubPlatform(name string) *stubPlatform {
	return &stubPlatform{
		name: name,
		exchangeToken: func(_ context.Context, _ string) (*xoauth2.Token, error) {
...
func makeHandler(userRepo *stubUserRepo) command.HandleCallbackHandler {
...
func TestHandleCallback_ExistingUser(t *testing.T) {
	...
	now := time.Now().UTC()
	existingUser, regErr := aggregate.RegisterUser(
		identityvo.UserName("existinguser"),
		identityvo.Email("existing@example.com"),
		identityvo.Avatar(""),
		constant.OAuthProviderGithub,
		"github-user-123",
		now,
	)
	if regErr != nil {
		t.Fatalf("RegisterUser() error: %v", regErr)
	}
	existingUser.SetID(42)

	repo := newStubUserRepo()
	repo.findByGithubBindID = func(_ context.Context, bindID string) (*aggregate.User, error) {
		if bindID == "github-user-123" {
	...
	repo.touchLastLogin = func(_ context.Context, userID uint) error {
		touchCalled = true
		if userID != 42 {
			t.Errorf("TouchLastLogin userID = %d, want 42", userID)
		}
		return nil
	}
	...
	result, err := handler.Handle(ctx, command.HandleCallbackCommand{
		Platform: constant.OAuthProviderGithub,
		Code:     "auth-code-456",
		State:    state,
	})
	if err != nil {
