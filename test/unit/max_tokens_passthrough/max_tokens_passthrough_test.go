// Package max_tokens_passthrough 回归测试：OpenAI /chat/completions forwardNative 路径
...
//
// 本测试直接用 service 层 body 构建所用的两步纯函数（proxy.ReplaceModelInBody +
...
// buildForwardBody 复刻 openAIService.forwardNative 中的 body 构建流程：
//  1. sonic.Marshal(req)
//  2. proxy.ReplaceModelInBody 替换 model 字段为上游真实 model
...
func buildForwardBody(t *testing.T, req *dto.OpenAIChatCompletionReq, upstreamModel string) []byte {
	...
	body := proxy.ReplaceModelInBody(raw, upstreamModel)
