// Package trigger_repository 验证 triggerRepository.UpdateAction 的存在性检查、
// 软删过滤与 updated_at 维护。
//
...
func mustCreate(t *testing.T, db *gorm.DB, word string) uint {
	t.Helper()
	m := &dbmodel.Trigger{Word: word, Action: "deny"}
	if err := db.Create(m).Error; err != nil {
		t.Fatalf("create trigger word failed: %v", err)
	}
	return m.ID
}
...
func TestUpdateAction_Success(t *testing.T) {
	t.Parallel()
	db := newTestDB(t)
	if err := db.AutoMigrate(&dbmodel.Trigger{}); err != nil {
...
func TestUpdateAction_SoftDeletedRejected(t *testing.T) {
	...
	id := mustCreate(t, db, "foo")
	if err := db.Model(&dbmodel.Trigger{}).Where("id = ?", id).Update("deleted_at", 1).Error; err != nil {
