...
func TestACmatcher_SingleWord(t *testing.T) {
	t.Parallel()
	m := trigger.NewACmatcher(map[uint]string{1: "hello"})
...
func TestACmatcher_NoMatch(t *testing.T) {
	t.Parallel()
	m := trigger.NewACmatcher(map[uint]string{1: "forbidden"})
	ids := m.Match("say hello world")
	if len(ids) != 0 {
...
func TestACmatcher_MultipleWords(t *testing.T) {
	t.Parallel()
	m := trigger.NewACmatcher(map[uint]string{
		1: "bad",
		2: "evil",
		3: "wicked",
	})
...
func TestACmatcher_CaseInsensitiveMixed(t *testing.T) {
	t.Parallel()
	m := trigger.NewACmatcher(map[uint]string{1: "badword"})
...
func TestACmatcher_SubstringMatch(t *testing.T) {
	t.Parallel()
	m := trigger.NewACmatcher(map[uint]string{1: "bad"})
...
func TestACmatcher_UnicodeText(t *testing.T) {
	t.Parallel()
	m := trigger.NewACmatcher(map[uint]string{1: "敏感"})
...
func TestACmatcher_DuplicateDedup(t *testing.T) {
	t.Parallel()
	m := trigger.NewACmatcher(map[uint]string{1: "aa"})
	ids := m.Match("aaa")
	if len(ids) != 1 || ids[0] != 1 {
