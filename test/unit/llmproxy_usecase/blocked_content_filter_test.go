...
func assertChatContentFilterBody(t *testing.T, body []byte) {
	...
	if choice.Message == nil || choice.Message.Content == nil || choice.Message.Content.Text != constant.BlockedContentFilterMessage {
		t.Fatalf("message.content = %+v, want %q", choice.Message, constant.BlockedContentFilterMessage)
	}
...
func assertResponseContentFilterBody(t *testing.T, body []byte) {
	...
	if part.Refusal == nil || *part.Refusal != constant.BlockedContentFilterMessage {
		t.Fatalf("content part refusal = %v, want %q", part.Refusal, constant.BlockedContentFilterMessage)
	}
}
...
func assertAnthropicContentFilterBody(t *testing.T, body []byte) {
	...
	if msg.StopDetails.Explanation == nil || *msg.StopDetails.Explanation != constant.BlockedContentFilterMessage {
		t.Fatalf("stop_details.explanation = %v, want %q", msg.StopDetails.Explanation, constant.BlockedContentFilterMessage)
	}
	if len(msg.Content) != 1 || msg.Content[0].Text == nil || *msg.Content[0].Text != constant.BlockedContentFilterMessage {
		t.Fatalf("content = %+v, want text block %q", msg.Content, constant.BlockedContentFilterMessage)
	}
}
...
// TestOpenAICreateChatCompletion_DenyBlockedUnary
// OpenAI Chat 非流式命中 deny 词时返回 200 content_filter JSON，不触达上游。
func TestOpenAICreateChatCompletion_DenyBlockedUnary(t *testing.T) {
	t.Parallel()
...
// TestOpenAICreateChatCompletion_DenyBlockedStream
// OpenAI Chat 流式命中 deny 词时返回 200 SSE：role → content → finish_reason=content_filter → [DONE]。
func TestOpenAICreateChatCompletion_DenyBlockedStream(t *testing.T) {
	t.Parallel()
	proxy := &mockOpenAIProxy{}
	resolver := &mockResolver{resolveEndpoint: buildTestEndpoint(), resolveModel: buildTestModel()}
	blocked := &fakeBlockedChecker{blockedIDs: []uint{1}, denyIDs: []uint{1}}
	...
	if second.Choices[0].Delta.Content == nil || *second.Choices[0].Delta.Content != constant.BlockedContentFilterMessage {
		t.Fatalf("content chunk delta = %+v, want %q", second.Choices[0].Delta, constant.BlockedContentFilterMessage)
	}
...
// TestOpenAICreateResponse_DenyBlockedStream
// /responses 流式命中 deny 词时返回 200 SSE 事件序列，completed 事件带 incomplete_details=content_filter。
func TestOpenAICreateResponse_DenyBlockedStream(t *testing.T) {
	t.Parallel()
	proxy := &mockOpenAIProxy{}
	resolver := &mockResolver{resolveEndpoint: buildTestEndpoint(), resolveModel: buildTestModel()}
	blocked := &fakeBlockedChecker{blockedIDs: []uint{1}, denyIDs: []uint{1}}
...
// TestAnthropicCreateMessage_DenyBlockedUnary
// Anthropic 非流式命中 deny 词时返回 200 refusal JSON（stop_reason=refusal），不触达上游。
func TestAnthropicCreateMessage_DenyBlockedUnary(t *testing.T) {
	t.Parallel()
	...
	blocked := &fakeBlockedChecker{blockedIDs: []uint{1}, denyIDs: []uint{1}}
	uc := usecase.NewAnthropicUseCase(mockResolver, &mockAnthropicListModels{}, &mockAnthropicCountTokens{}, mockProxy, &mockOpenAIProxy{}, &mockTaskSubmitter{}, blocked, nil)
...
// TestAnthropicCreateMessage_DenyBlockedStream
// Anthropic 流式命中 deny 词时返回 200 SSE 事件序列，message_delta 带 stop_reason=refusal。
func TestAnthropicCreateMessage_DenyBlockedStream(t *testing.T) {
	t.Parallel()
	mockProxy := &mockAnthropicProxyForAnthropic{}
	mockResolver := &mockResolver{resolveEndpoint: buildAnthropicTestEndpoint(), resolveModel: buildAnthropicTestModel()}
	blocked := &fakeBlockedChecker{blockedIDs: []uint{1}, denyIDs: []uint{1}}
	...
	if delta.Delta.Text != constant.BlockedContentFilterMessage {
		t.Fatalf("content_block_delta text = %q, want %q", delta.Delta.Text, constant.BlockedContentFilterMessage)
	}
