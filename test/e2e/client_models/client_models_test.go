...
func TestClientListModels_ReturnsTrimmedItems(t *testing.T) {
	...
	huma.Register(api, huma.Operation{
		OperationID: "listClientModels", Method: http.MethodGet, Path: "/api/v1/model/list",
		Tags: []string{constant.TagClient},
	}, h.HandleListModels)

	req := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/api/v1/model/list", http.NoBody)
...
func TestClientModels_E2E(t *testing.T) {
	...
	defer cancel()
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, strings.TrimRight(baseURL, "/")+"/api/v1/model/list", http.NoBody)
	if err != nil {
		t.Fatal(err)
	}
