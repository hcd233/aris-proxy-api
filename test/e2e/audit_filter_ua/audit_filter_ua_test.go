...
func doListAuditLogs(t *testing.T, client *http.Client, baseURL, jwtToken, filter string) (int, *listAuditLogsRsp) {
	t.Helper()
	q := url.Values{}
	q.Set("page", "1")
	q.Set("pageSize", "20")
...
func doListAuditOptions(t *testing.T, client *http.Client, baseURL, jwtToken, field string) (int, *optionListRsp) {
	t.Helper()
	q := url.Values{}
	q.Set("field", field)
...
// TestAuditListFilterUA_NoCrash 验证各种 ua filter 表达式不触发 500 且返回正常结构。
// 覆盖含空格的引号包裹 UA（如 Mozilla/5.0 (Macintosh)）与多值 | 连接。
//
//	@author centonhuang
//	@update 2026-08-10 16:00:00
func TestAuditListFilterUA_NoCrash(t *testing.T) {
	t.Parallel()
	...
	filters := []string{
		...
		`ua:"Mozilla/5.0 (Macintosh)"|curl/8.0`,
		`ua:curl/8.0`,
	}
	...
	for _, f := range filters {
		...
		t.Run(fmt.Sprintf("filter=%q", f), func(t *testing.T) {
			t.Parallel()
			status, body := doListAuditLogs(t, client, baseURL, jwtToken, f)
...
//	@author centonhuang
//	@update 2026-08-10 16:00:00
func TestAuditListFilterUA_OptionList(t *testing.T) {
	t.Parallel()
	baseURL, jwtToken := mustEnv(t)
	client := newClient()

	status, body := doListAuditOptions(t, client, baseURL, jwtToken, "ua")
	if status != http.StatusOK {
