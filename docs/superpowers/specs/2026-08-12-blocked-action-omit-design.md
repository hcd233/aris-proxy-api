...
Blocked Words（敏感词）的命中处理动作 `action` 当前枚举为 `deny` / `allow`。`allow` 的语义是「命中放行，但不记录 session/message/tool」——即省略存储，而非一般意义的"允许"。将 `allow` 更名为 `omit`，语义更准确地表达「命中即省略落库」，避免与"放行/允许"歧义。

## 领域语义（沿用 CONTEXT.md 词汇表）

- **Blocked（敏感词）**：管理员配置的敏感词黑名单条目，含 `word`、`action`（`deny` 拦截 / `omit` 省略）、`hitCount`。
...
| D4 | 前端文案 | zh「省略不记录」/ en「Omit (no record)」/ ja「省略（記録なし）」；i18n key `blocked.action_allow` → `blocked.action_omit` | 语义对齐（用户指定） |
...
1. **枚举**（`internal/common/enum/blocked_action.go`）
   - `BlockedActionAllow` → `BlockedActionOmit = "omit"`，注释改「命中省略，不记录 session/message/tool」
...
9. `web/src/locales/zh.json` / `en.json` / `ja.json`：`blocked.action_allow` → `blocked.action_omit`（zh「省略不记录」/ en「Omit (no record)」/ ja「省略（記録なし）」）

### 词汇表

10. `CONTEXT.md`：Blocked 术语 `action` 描述 `allow` → `omit`，语义「命中放行，但不落库」→「命中省略，不落库」
