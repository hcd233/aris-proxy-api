...
```
GET /api/v1/session/messages?sessionId={id}&offset={n}&limit={m}
认证: jwtAuth
权限: PermissionUser
```
...
```
GET /api/v1/session/tools?sessionId={id}&offset={n}&limit={m}
```
...
type SessionDependencies struct {
    ...
    PageMessages     sessionquery.PageSessionMessagesHandler        // 新增
    PageTools        sessionquery.PageSessionToolsHandler           // 新增
...
| `test/unit/session_meta_query/` | `getSessionMetaByUserHandler` | 缓存命中、缓存未命中回源、权限拒绝、404 |
| `test/unit/session_messages_query/` | `pageSessionMessagesHandler` | offset 越界、limit 边界、partial cache hit、空 messageIDs |
| `test/unit/session_tools_query/` | `pageSessionToolsHandler` | 同上 |
| `test/unit/session_dto/` (扩) | 新 DTO 序列化 | metadata、page response、错误响应 |
...
- `TestSessionDetailPerf_GetMetadata_Success`：metadata 接口返回正确字段
- `TestSessionDetailPerf_GetMetadata_NoPermission`：他人 session → 403/无权限错误
- `TestSessionDetailPerf_PageMessages_Pagination`：offset+limit 正确切片，total 等于 messageIDs 长度
- `TestSessionDetailPerf_PageMessages_LimitRejected`：limit > 100 → 400 校验错误（不 clamp）
- `TestSessionDetailPerf_PageTools_Pagination`：同上
...
- 长 session（≥ 50 messages）下分页接口的耗时分布。
...
| Step 2: query handler 落地 | `test/unit/session_meta_query/`、`session_messages_query/`、`session_tools_query/` 全绿 |
...
| Step 5: 部署后线上验证 | 用 `call-api` skill 跑 metadata/messages/tools 三个接口；任一失败按 `cls-log-bugfix` 流程查 traceId |
