...
- 模型按 `id` 合并；本次选中的模型会替换目标 provider 中相同 ID 的全部已有
  模型，未选中的已有模型继续保留。
- Provider ID、基础地址和 API 密钥通过 Shell 变量暴露，并可使用
  `PROVIDER_ID`、`BASE_URL`、`API_KEY` 覆盖。
- 生成脚本中的 JSON 操作只使用 Python 标准库。
- 配置文件、备份和临时文件使用 `0600` 权限，默认配置目录使用 `0700` 权限。
- 通过同目录临时文件写入并使用 `os.replace` 原子替换配置，避免中断时截断原文件。
- 选中的模型存在重复 `alias` 时不生成脚本，并在界面提示用户每个 alias 只能选择一个模型。
