...
func (dao *MinioObjDAO) PresignObject(ctx context.Context, userID uint, objectName string) (presignedURL *url.URL, err error) {
	...
	reqParams.Set(constant.HTTPLowerHeaderContentDisposition, fmt.Sprintf(constant.HTTPAttachmentFilenameTemplate, filepath.Base(objectName)))
	...
	reqParams.Set(constant.HTTPLowerHeaderContentType, contentType)
