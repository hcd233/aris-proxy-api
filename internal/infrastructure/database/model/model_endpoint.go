...
type ModelEndpoint struct {
	...
	Alias     string `json:"alias" gorm:"column:alias;not null;uniqueIndex:idx_alias_provider_deleted,priority:1;comment:模型别名（对外暴露的名称）"`
	Model     string `json:"model" gorm:"column:model;not null;comment:上游实际模型名称"`
	Provider  string `json:"provider" gorm:"column:provider;not null;uniqueIndex:idx_alias_provider_deleted,priority:2;comment:协议提供方(openai/anthropic)"`
	...
	DeletedAt int64  `json:"deleted_at" gorm:"column:deleted_at;default:0;uniqueIndex:idx_alias_provider_deleted,priority:3;comment:删除时间，默认为0"`
}
