...
func (h *updateModelHandler) Handle(ctx context.Context, cmd port.UpdateModelCommand) (llmproxy.ModelIDSyncCounts, error) {
	...
	if m == nil {
		return llmproxy.ModelIDSyncCounts{}, ierr.New(ierr.ErrDataNotExists, "model not found")
	}
	...
	if cmd.EndpointID != nil {
		...
		if eerr != nil {
			...
			return llmproxy.ModelIDSyncCounts{}, eerr
		}
		if ep == nil {
			return llmproxy.ModelIDSyncCounts{}, ierr.New(ierr.ErrDataNotExists, "endpoint not found")
		}
		if ep.UserID() != m.UserID() {
			return llmproxy.ModelIDSyncCounts{}, ierr.New(ierr.ErrNoPermission, "endpoint owner does not match model owner")
		}
	}
