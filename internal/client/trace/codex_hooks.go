...
// codexIngestCommand 返回 codex hook 注册用的完整 ingest 命令（显式携带 --agent codex）
func codexIngestCommand(binPath string) string {
	return binPath + constant.TraceClientIngestCommandSuffix + " --agent " + constant.TraceAgentCodex
}
...
func InstallCodexHooks(paths Paths, binPath string) (int, error) {
	...
	command := codexIngestCommand(binPath)
