...
const (
	...
	// AnthropicProxyZonePrefix anthropic 分区前缀（不含版本段）：服务端路由前缀
	// AnthropicProxyPrefix 与 Claude Code 的 base URL 都从它派生，避免「必须少一段 /v1」
	// 这类不变量只靠注释维系（2026-09-15 CR）。
	AnthropicProxyZonePrefix = "/api/anthropic"
	AnthropicProxyPrefix     = AnthropicProxyZonePrefix + "/v1"

	// ClaudeCodeBaseURLPrefix 写入 Claude Code 的 ANTHROPIC_BASE_URL 前缀，即分区前缀本身。
	// Claude Code（Anthropic SDK）在 base URL 之后追加 /v1/messages，故这里必须比网关路由
	...
	ClaudeCodeBaseURLPrefix = AnthropicProxyZonePrefix
