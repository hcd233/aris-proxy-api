...
func (p *openAIProxy) ForwardCreateResponse(ctx context.Context, ep UpstreamEndpoint, body []byte) ([]byte, error) {
	...
	respBody, err := io.ReadAll(resp.Body)
	if err != nil {
		logger.Error("[OpenAIProxy] Read response api upstream response error", zap.Error(err))
		return nil, &model.UpstreamConnectionError{Cause: err}
	}
...
func (p *openAIProxy) ForwardCreateResponseStream(ctx context.Context, ep UpstreamEndpoint, body []byte, onEvent func(event string, data []byte) error) error {
	...
	for {
		...
		if line != "" {
			if strings.HasPrefix(line, constant.SSEEventPrefix) {
				...
				if err := onEvent(currentEvent, []byte(payload)); err != nil {
					logger.Warn("[OpenAIProxy] Response API onEvent callback error", zap.Error(err))
					return err
				}
				currentEvent = ""
			}
		...
		if readErr != nil {
			if readErr != io.EOF {
				logger.Warn("[OpenAIProxy] Read response api upstream sse error", zap.Error(readErr))
...
func (p *openAIProxy) sendResponseRequest(ctx context.Context, ep UpstreamEndpoint, body []byte) (*http.Response, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, upstreamURL, bytes.NewReader(body))
	if err != nil {
		logger.Error("[OpenAIProxy] New response api request error", zap.String("upstreamURL", upstreamURL), zap.Error(err))
		return nil, ierr.Wrap(ierr.ErrProxyRequest, err, "create response api request")
	}
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+ep.APIKey)

	logger.Info("[OpenAIProxy] Send response api upstream request", zap.String("upstreamURL", upstreamURL),
		zap.String("upstreamModel", ep.Model),
		zap.Any("upstreamAPIKey", util.MaskSecret(ep.APIKey)))

	resp, err := httpclient.GetHTTPClient().Do(req)
	if err != nil {
		logger.Error("[OpenAIProxy] Send response api http request error", zap.String("upstreamURL", upstreamURL), zap.Error(err))
		return nil, &model.UpstreamConnectionError{Cause: err}
	}
	...
	if resp.StatusCode != http.StatusOK {
		errorBody, _ := io.ReadAll(resp.Body)
		_ = resp.Body.Close()
		logger.Error("[OpenAIProxy] Response API upstream returned non-200 status",
			zap.String("upstreamURL", upstreamURL),
			zap.Int("statusCode", resp.StatusCode),
			zap.String("responseBody", string(errorBody)),
		)
